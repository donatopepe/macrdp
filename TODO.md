...
- [~] **Live socket-owner adapter** — blocked safely at design boundary: current
  ...
  path. Owner now also exposes a typed status snapshot, zero-value status constructor, explicit owner-state enum, and pure lifecycle transition helper for the live integration harness.

...
- [~] **Live socket-owner adapter** — blocked safely at design boundary: current
  ...
  drain, writer-error, partial-write, all-class typed ingress, ingress counters/snapshot, per-class
  byte occupancy, pre-framing upper-bound admission, audio-admission debug guard, and fake-writer
  tests; it awaits each write

...
- [~] **Live socket-owner adapter** — blocked safely at design boundary: current
  ...
  drain-state (tested), explicit ingress-batch budget, observable/exhausted/reset urgent budget, writer-error, partial-write, all-class typed ingress, ingress counters/snapshot accessors,
  owner scheduler snapshots with ingress counters and queue-depth test,
